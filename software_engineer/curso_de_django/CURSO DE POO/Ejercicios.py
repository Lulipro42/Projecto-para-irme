...
"""
EJERCICIO: TRACKER DE FINANZAS 💰 (EXPLICADO PASO A PASO)
"""
...
"""
SISTEMA DE STREAMING "GEMINI-FLIX" 📺
-----------------------------------
Este código integra: Clases, Listas, Bucle For, Validaciones y Manejo de Errores.
"""

# 1. Definimos la estructura del dato (como una fila en una tabla)
class Contenido:
...
# Paso A: Creamos las herramientas
netflix = Plataforma()
ulises_critico = Critico()

# Paso B: Creamos el contenido (incluimos uno con error a propósito)
peli1 = Contenido("Batman", "Acción", 9)
peli2 = Contenido("Titanic", "Drama", 7)
peli3 = Contenido("Inception", "Acción", 10)
peli4 = Contenido("Peli Trucha", "Comedia", 15) # Esto debería fallar

# Paso C: Subimos a la plataforma
netflix.subir_contenido(peli1)
netflix.subir_contenido(peli2)
netflix.subir_contenido(peli3)
netflix.subir_contenido(peli4) # Aquí saltará el "Error de rango"

# Paso D: Probamos los métodos de búsqueda y análisis
...
nombres_vistos = [] # Acá guardamos solo los nombres (strings) para comparar rápido
datos_unicos = []   # Acá guardamos la fila completa [Nombre, Posicion, Puntos]

with open("voley_data.csv", "r", encoding="utf-8") as archivo:
    lector = csv.reader(archivo)
    encabezados = next(lector)
    datos_unicos.append(encabezados) # Guardamos los títulos primero
    
    for fila in lector:
        nombre = fila[0] # Sacamos el nombre de la columna 0
        
        # PREGUNTA CLAVE: ¿El nombre NO está en nuestra lista de vistos?
        if nombre not in nombres_vistos:
            nombres_vistos.append(nombre) # Lo anotamos para la próxima
            datos_unicos.append(fila)     # Guardamos toda la fila como "única"

# Al final, guardamos los que pasaron el filtro
